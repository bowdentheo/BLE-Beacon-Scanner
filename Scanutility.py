#Updated for Eddystone TLM and EID
    ...
    for i in range(0, loop_count):
        ...
        if (dataString[34:42] == '0303aafe'):
            ...
            if broadcastType == '00' :
                ...
                type = "Eddystone TLM"
                voltage = int(dataString[54:58], 16)
                temperature = int(dataString[58:62], 16)
                adv_count = int(dataString[62:70], 16)
                sec_count = int(dataString[70:78], 16)
                resultsArray = [
                    {"type": type, "voltage": voltage, "temperature": temperature, "adv_count": adv_count, "sec_count": sec_count}]
                return resultsArray
          
            elif broadcastType == '30':
                type = "Eddystone EID"
                # Extract Ephemeral Identifier (EID) data as needed
                # Example: eid_data = dataString[54:74]
                # Update the resultsArray accordingly
                resultsArray = [{"type": type, "eid_data": eid_data}]
                return resultsArray
